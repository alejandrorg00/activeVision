...
def plot_attention_exploration(
    ...
    This is meant to be called after run_attention(...).
    ...
    ----------
    events_npy:
        Path to events.npy with columns x, y, polarity, time_s.

    saccades_path:
        Path to saccades.dat generated by run_attention.

    output_dir:
        Directory where the PNG/PDF will be saved.

    resolution:
        Optional (H, W). If None, inferred from events.

    beta:
        Current beta value, only used for title/printing.

    per:
        Grid cell size as fraction of image width/height.
        per=0.1 means cells are 10% of image size.

    noise_thresh:
        Minimum number of events in a cell to count it as object cell.
    """
